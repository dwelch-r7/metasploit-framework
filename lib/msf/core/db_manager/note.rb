module Msf::DBManager::Note
  ...
  def report_notes(notes)
    ...
    ::ApplicationRecord.connection_pool.with_connection {
      ...
      serialized_notes = mdm_notes.map do |x|
      ...
      ids = Mdm::Note.upsert_all(serialized_notes)

      # Manually assign the bulk IDs, and run save callbacks
      mdm_notes.zip(ids).each do |note, id|
        note.id = id
